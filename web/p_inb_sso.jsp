...
<%
    ...
    response.setHeader("Content-Disposition", "filename=\"p-bb-sso.jnlp\";");
...
<jnlp spec="1.0+" codebase="<%= codebase %>" href="">
    ...
    <applet-desc
        ...
        <param name="serverURL"
               value="/forms/lservlet?ifcfs=/forms/frmservlet?config=<%= request.getParameter("config") %>&#38;otherParams=iamticket=<%= iamticket%>&#38;ifsessid=WLS_FORMS.formsapp.1625&#38;acceptLanguage=en-US,en;q=0.8,en-CA;q=0.6"/>
        <param name="networkRetries" value="0"/>
        <param name="iamticket" value="<%= iamticket %>"/>
        <param name="serverArgs"
               value="escapeParams=true module=guainit.fmx debug=no host= port= obr=no record= tracegroup= log= term= iamticket=<%= iamticket%>"/>
